...
@Query("select sum(e.ccy_Amt) from Install_paid_exceed_limit_UP e where e.acct_Key=?1")
	public Double CalculateTransactionTotalAmount(int accountKey);
 @Query("select count(e.acct_Key)  from Install_paid_exceed_limit_UP e where e.acct_Key=?1")
	public Integer CalculateTransactionsCount(int accountKey);
 @Query("select count(e.acct_Key)  from Install_paid_exceed_limit_UP e where e.acct_Key=?1")
	public Double CalculateInstallmentNumber(int accountKey);
