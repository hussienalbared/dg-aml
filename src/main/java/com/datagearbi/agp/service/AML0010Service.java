...
public class AML0010Service {
	...
	public List<AlarmDTO> getAllRecordsFromView() {
		...
		int date_differnce=this.getm010_num_days();
		List<ExcessiveAccountActivities> records = this.excessiveAccountActivitiesRepository
				.findByCdiCodeInAndAccttypedescInAndPrimMedDescInAndDateDiffLessThan(cdi_code, AccountTypes, primMedDesc,date_differnce);
		...
		records.forEach(res -> {
			...
			temp.setDate_diff(String.valueOf(res.getDateDiff()));
	...
	public List<AcRoutineHelper> getRoutine_10_Parameters() {
		List<AcRoutineHelper> routine_detail = this.ac_RoutineRepository.getRoutineDetail("AML010");
	...
	public Map<String, List<AlarmDTO>> generateaAlarms() {
		...
		alarmDtos.forEach((a, b) -> {
			...
			for (AlarmDTO x : b) {
				try {
					aggAmount += Double.valueOf(x.getCcy_Amt());
			...
			if (aggAmount >= this.getm010_amount() && activityNumber >= this.getm010_num_events()) {
				...
				alramInsertionUtil.setPrimary_obj_key(Integer.parseInt(a));
...
private int getm010_num_days() {
	List<String> values = this.routine_ParameterRepository.getParamValueByParamName("m010_num_days");
	int amount = 365;
	if (values.size() > 0 && (values.get(0) != null && values.get(0).length() != 0)) {
		try {
			amount = Integer.valueOf(values.get(0));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
